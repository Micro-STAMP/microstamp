...
import { IConnectionActionType, IConnectionReadDto, IControlAction } from "@interfaces/IStep2";
	...
	try {
		...
		const controlActions: IControlAction[] = connections.flatMap(connection =>
			connection.connectionActions
				.filter(
					action => action.connectionActionType === IConnectionActionType.CONTROL_ACTION
				)
				.map(action => ({
					id: action.id,
					name: action.name,
